...
def _plot_images_absolute(
    ...
    Args:
        path: absolut path for which duplicates are to be plotted.
        image_list: List of duplicate filenames, could also be with scores (filename, score).
        scores: Whether only filenames are present in the image_list or scores as well.
        outfile:  Name of the file to save the plot.
    """
    ...
    ax.set_title('Original Image: {}'.format(path))
    ax.axis('off')
...
def plot_duplicates_multi(
    ...
    Args:
        image_dirs: image directories where all files in duplicate_map are present.
        duplicate_map: mapping of filename to found duplicates (could be with or without scores).
        path: Absolute path of the file for which duplicates are to be plotted, must be a key in the duplicate_map.
        dictionary.
        outfile: Optional, name of the file to save the plot. Default is None.
    ...
    """
    print(duplicate_map)
    retrieved = duplicate_map[path]
    assert len(retrieved) != 0, 'Provided filename has no duplicates!'

    # plot
    if isinstance(retrieved[0], tuple):
        _plot_images_absolute(
            path=path,
            image_list=retrieved,
            scores=True,
            outfile=outfile,
