...
class CNN:
    ...
    def __init__(self, verbose: bool = True) -> None:
        ...
        from imagededup.utils.data_generator_multi import DataGeneratorMulti

        self.MobileNet = MobileNet
        self.preprocess_input = preprocess_input
        self.DataGenerator = DataGenerator
        self.DataGeneratorMulti = DataGeneratorMulti
    ...
    def _get_cnn_features_batch_multi(self, image_dirs: List[Path]) -> Dict[str, np.ndarray]:
        """
        Generate CNN encodings for all images in a given directory of images.
        Args:
        ...
        """
        self.logger.info('Start: Image encoding generation')
        self.data_generator_multi = self.DataGeneratorMulti(
        ...
        feat_vec = self.model.predict_generator(
            self.data_generator_multi, len(self.data_generator_multi), verbose=self.verbose
        )
        self.logger.info('End: Image encoding generation')

        filenames = [str(i.absolute()) for i in self.data_generator_multi.valid_image_files]
    ...
    def encode_images_multi(self, image_dirs: List[Union[PurePath, str]]) -> Dict:
        """Generate CNN encodings for all images in a given directory of images.
        ...
        """
        dir_path = []
        for _, image_dir in enumerate(image_dirs):
            if isinstance(image_dir, str):
                dir_path.append(Path(image_dir))
                image_dir = Path(image_dir)

            if not image_dir.is_dir():
                raise ValueError('Please provide a valid directory path!')

        return self._get_cnn_features_batch_multi(dir_path)
    ...
    def _find_duplicates_dict(
        ...
        for i, j in enumerate(self.cosine_scores):
            ...
            for duplicate in duplicates:
                self.generalDuplicates.add(duplicate)

            if image_ids[i] not in self.generalDuplicates:
